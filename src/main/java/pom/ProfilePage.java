...
public class ProfilePage {
    ...
    private final By LOGOUT_BUTTON = By.xpath("//button[contains(text(),'Выход')]");
    private final By PROFILE_LABEL = By.xpath(
    ...
    public void clickLogoutButton() {
        WebElement button = wait.until(ExpectedConditions.elementToBeClickable(LOGOUT_BUTTON));
        button.click();
    }

    @Step("проверка отображения заголовка 'Профиль'")
    public boolean checkLabelProfileIsVisible() {
        WebElement label = wait.until(ExpectedConditions.elementToBeClickable(PROFILE_LABEL));
